...
# all *.$rancher_domain which arent in /etc/hosts are forward to our pdns server.
server=/$rancher_domain/127.0.0.2
server=$default_dns_resolver
EOF
rm /etc/resolv.conf
cat >/etc/resolv.conf <<EOF
nameserver 127.0.0.1
search $rancher_domain
EOF
...
wget -qO- --header 'X-API-Key: vagrant' http://$pandora_ip_address:8081/api/v1/servers/localhost/zones/$rancher_domain | jq .
