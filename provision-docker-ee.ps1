param(
    [string]$dockerVersion = '19.03'
)

# normalize the version to <major>.<minor> as thats the only format supported by DockerMsftProvider.
$dockerVersion = $dockerVersion -replace '^(\d+\.\d+).*$','$1'
